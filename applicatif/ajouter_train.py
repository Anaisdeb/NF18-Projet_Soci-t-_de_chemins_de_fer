...
def ajout(connexion) :
    ...
    row = cur.fetchone()
    if row == None :
        print("Ajoutez des lignes dans votre BDD")
    else :
        while row:
            print()
        ...
        l=bool(input("Voulez-vous que le train circule en semaine (du lundi au vendredi) ? (True/False) "))
        ...
        while j!=0 :
            ...
            row = cur.fetchone()
            if row == None :
                print("Il n'y a aucun arrêt sur cette ligne")
                j=0
            else :
                i=1
                print()
                ...
                try:
                    ...
                    if(error_managing.erreurLigne(connexion)):
                        print("Erreur d'insertion : l'arrêt affecté au train ne se situe pas sur la ligne sur laquelle il circule !")
                        connexion.rollback()
                    else:
                        connexion.commit()
                except psycopg2.DataError as e:
